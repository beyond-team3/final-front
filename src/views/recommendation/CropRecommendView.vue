...
// TODO: API 연결
const monthData = ref([
  { month: 1, summary: '저온기 관리에 강한 품종 중심으로 추천합니다.', tags: ['저온기', '초세안정', '하우스'], matchTerms: ['저온강', '고랭지', '비닐하우스'] },
  { month: 2, summary: '정식 준비 시즌, 생육 밸런스가 좋은 품종을 추천합니다.', tags: ['정식준비', '활착', '생육균형'], matchTerms: ['조생종', '강세', '노지'] },
  { month: 3, summary: '본격 파종/정식 시작, 수량성과 균일도가 좋은 품종을 추천합니다.', tags: ['파종', '정식', '균일도'], matchTerms: ['고수량', '기계수확가능', '노지'] },
  { month: 4, summary: '기온 상승기, 병해 리스크를 고려한 품종을 추천합니다.', tags: ['병해', '내병성', '환기'], matchTerms: ['탄저병', '역병', '노균병', '바이러스'] },
  { month: 5, summary: '생육이 빠른 시기, 관리 편의성이 높은 품종을 추천합니다.', tags: ['관리', '생육속도', '하우스'], matchTerms: ['장기수확', '조생종', '비닐하우스'] },
  { month: 6, summary: '고온기 초입, 스트레스 대응 품종을 추천합니다.', tags: ['고온기', '스트레스', '활착'], matchTerms: ['중생종', '노지', '바이러스'] },
  { month: 7, summary: '고온다습, 병해 저항성을 우선한 품종을 추천합니다.', tags: ['고온다습', '병해저항', '환기'], matchTerms: ['노균병', '바이러스', '비닐하우스'] },
  { month: 8, summary: '작기 전환기, 회복력이 좋은 품종을 추천합니다.', tags: ['작기전환', '회복력', '균일도'], matchTerms: ['중생종', '저장성우수', '상품률우수'] },
  { month: 9, summary: '가을작기 진입, 수량성과 상품성이 좋은 품종을 추천합니다.', tags: ['가을작기', '상품성', '수량성'], matchTerms: ['고수량', '당도우수', '매운맛우수'] },
  { month: 10, summary: '일교차 확대, 내한성과 생육 안정 품종을 추천합니다.', tags: ['일교차', '내한성', '안정'], matchTerms: ['저온강', '고랭지', '중생종'] },
  { month: 11, summary: '저온기 시작, 초기 생육이 안정적인 품종을 추천합니다.', tags: ['저온기', '초기생육', '하우스'], matchTerms: ['저온강', '조생종', '비닐하우스'] },
  { month: 12, summary: '연중 최저온, 관리 난이도가 낮은 품종을 추천합니다.', tags: ['최저온', '관리용이', '안정'], matchTerms: ['기계수확가능', '저장성우수', '노지'] },
])
...
const selectedProducts = computed(() => {
  ...
  const terms = selectedInfo.value.matchTerms || []
  ...
  const matched = allProducts.filter((product) => {
    const text = [
      ...
      product.category,
      product.desc,
      ...(Object.values(product.tags || {}).flat()),
    ]
      .filter(Boolean)
      .join(' ')
      .toLowerCase()
  ...
  // Fallback: keep monthly variation even when no term matches.
  const offset = (selectedMonth.value - 1) % Math.max(allProducts.length, 1)
  return [...allProducts.slice(offset), ...allProducts.slice(0, offset)].slice(0, 3)
})
    ...
    <div class="cultivate-wrap">
      ...
      <div class="product-panel">
        <div class="product-panel-header">
          <div class="product-panel-title">{{ selectedInfo.month }}월 추천 상품 리스트 (카탈로그 기준)</div>
        </div>
...
.empty-state { grid-column: 1 / -1; padding: 24px; text-align: center; border: 1px dashed #dfe6e9; border-radius: 10px; color: #7f8c8d; }
