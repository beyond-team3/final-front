...
const baseCandidates = computed(() => {
  const q = baseKeyword.value.trim().toLowerCase()
  if (!q) return []
  return productStore.products.filter((item) => item.name.toLowerCase().includes(q)).slice(0, 5)
})
...
const slotProducts = computed(() => {
  const filled = productStore.compareProducts
  return [filled[0] || null, filled[1] || null, filled[2] || null]
})

const addToCompare = (productId) => {
  const result = productStore.addCompareItem(productId)
...
const addNextToSlot = () => {
  if (!baseProduct.value) {
    window.alert('기준 상품을 먼저 선택해주세요.')
    return
  }

  const candidate = similarProducts.value.find((item) => !productStore.isInCompare(item.id))
  if (!candidate) {
...
const removeFromSlot = (productId) => {
  productStore.removeCompareItem(productId)
}

const resetAll = () => {
  productStore.clearSelectedBaseProduct()
  productStore.clearCompareItems()
  productStore.resetSimilarityOptions()
  baseKeyword.value = ''
}

const selectBaseProduct = () => {
  const q = baseKeyword.value.trim().toLowerCase()
  if (!q) {
    return
  }

  const exact = productStore.products.find((item) => item.name.toLowerCase() === q)
  const fallback = baseCandidates.value[0]
  const target = exact || fallback

  if (!target) {
    window.alert('검색된 기준 상품이 없습니다.')
    return
  }

  productStore.setSelectedBaseProduct(target.id)
  baseKeyword.value = target.name
}

const removeBaseProduct = () => {
  productStore.clearSelectedBaseProduct()
  productStore.clearCompareItems()
}
...
<template>
  <section>
    ...
    <section class="mb-5 rounded-xl border border-slate-200 bg-white p-5">
      <h3 class="mb-3 text-base font-bold text-slate-800">1. 기준 상품 + 기준 상품 정보</h3>

      <div class="grid gap-3 lg:grid-cols-[1fr_auto]">
        <div>
          <label class="mb-2 block text-sm font-semibold text-slate-700">기준 상품</label>
          <div class="flex gap-2">
            <input
              v-model="baseKeyword"
              list="base-product-options"
              type="text"
              class="h-11 w-full rounded border border-slate-300 px-3 text-sm"
              placeholder="상품명 검색"
              @keyup.enter="selectBaseProduct"
            >
            <button
              type="button"
          ...
          <datalist id="base-product-options">
            <option v-for="item in baseCandidates" :key="item.id" :value="item.name" />
          </datalist>

          <div v-if="baseProduct" class="mt-3 flex items-center justify-between rounded border border-slate-200 bg-slate-50 px-3 py-2">
            <div>
              <p class="text-sm font-semibold text-slate-800">{{ baseProduct.name }}</p>
              <p class="text-xs text-slate-500">{{ baseProduct.category }} · {{ baseProduct.id }}</p>
            </div>
            <button type="button" class="h-7 w-7 rounded-full bg-slate-200 text-sm text-slate-700" @click="removeBaseProduct">×</button>
          </div>
        </div>
        ...
        <div class="flex items-end">
          <button
            type="button"
            class="h-11 rounded border-2 border-red-400 px-5 text-sm font-semibold text-red-600 hover:bg-red-50"
            @click="resetAll"
          >
            전체 초기화
          </button>
      ...
      <div v-if="baseProduct" class="mt-4 rounded border border-slate-200 bg-slate-50 p-4">
        <h4 class="text-base font-bold text-slate-800">{{ baseProduct.name }}</h4>
        <p class="mt-1 text-sm text-slate-500">{{ baseProduct.desc }}</p>
        <div class="mt-4 grid gap-3 md:grid-cols-5">
          <div v-for="row in criteriaRows" :key="row.key" class="rounded border border-slate-200 bg-white p-3">
    ...
    <section class="mb-5 grid gap-5 xl:grid-cols-[280px_1fr]">
      <article class="rounded-xl border border-slate-200 bg-white p-4">
        <h3 class="mb-4 text-base font-bold text-slate-800">2. 유사도 분석 기준 설정</h3>
        <div class="space-y-2">
          ...
          <svg viewBox="0 0 800 450" class="h-[420px] w-full min-w-[700px]">
            ...
            >
              <circle
                ...
                :fill="node.similarity >= 70 ? '#4facfe' : '#95a5a6'"
                :stroke="node.similarity >= 70 ? '#00b7ff' : '#7f8c8d'"
                stroke-width="2"
              />
    ...
    <section class="rounded-xl border border-slate-200 bg-white p-4">
      <h3 class="mb-3 text-base font-bold text-slate-800">3. 비교 상품 선택 + 상품 비교 결과</h3>
