            ...
            <div v-if="!isClient" class="mt-3">
              ...
              <div class="mt-2 flex items-center gap-3">
                <span class="rounded px-2 py-1 text-xs font-semibold"
                  :class="product.status === 'SALE' || !product.status ? 'bg-[var(--color-olive-light)] text-[var(--color-olive-dark)]' : 'bg-[#F0D4D4] text-[var(--color-status-error)]'"
                >
                  {{ product.status === 'SOLDOUT' ? '일시 품절' : (product.status === 'STOP' ? '단종' : '판매 중') }}
                </span>
              </div>
            </div>
          </div>
