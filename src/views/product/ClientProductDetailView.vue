  ...
  <section v-if="product">
      ...
      <div class="flex flex-col">
        <div class="flex items-start justify-between gap-3 border-b border-slate-200 pb-4">
          <div>
            ...
            <div v-if="!isClient" class="mt-3 flex items-baseline gap-2">
    ...
    >
      <div class="mb-3 flex items-center justify-between gap-2">
        <h3 class="text-base font-bold text-slate-800">나의 영업 피드백</h3>
        <button
          type="button"
          class="rounded bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 transition-colors"
          @click="saveNote"
        >
          피드백 저장
        </button>
      ...
      <textarea
        v-model="noteDraft"
        class="h-32 w-full rounded-lg border border-slate-300 p-4 text-sm focus:border-blue-500 focus:outline-none"
        placeholder="해당 품종에 대한 영업 현장의 의견이나 고객 반응을 자유롭게 기록하세요."
      />
    </section>
  </section>

  <section v-else class="rounded-xl border border-slate-200 bg-white p-12 text-center text-sm text-slate-500">
