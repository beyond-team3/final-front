...
<template>
  <section class="min-h-screen bg-[var(--color-bg-base)] p-4 lg:p-8">
    ...
    <div v-if="showDetailModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="bg-[var(--color-bg-card)] rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col z-10 animate-in slide-in-from-bottom-8 duration-300">
        <div class="flex justify-between items-center p-6 border-b border-[var(--color-border-divider)] bg-[var(--color-bg-section)]">
          <div class="flex items-center gap-4">
            <h3 class="text-2xl font-bold text-[var(--color-text-strong)]">영업 활동 원문</h3>
            <span v-if="selectedNote?.isEdited" class="rounded-full bg-[var(--color-orange-light)] px-3 py-0.5 text-[10px] font-bold text-[var(--color-orange-dark)] uppercase">수정됨</span>
          </div>
          <button @click="closeDetail" class="h-10 w-10 flex items-center justify-center rounded-full hover:bg-[var(--color-bg-base)] transition-colors text-2xl text-[var(--color-text-sub)]">&times;</button>
        </div>
        <div class="p-8 lg:p-10 overflow-y-auto">
          <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 border-b border-[var(--color-border-divider)] pb-6 gap-4">
            <div>
              <p class="text-3xl font-bold text-[var(--color-olive)] mb-2">{{ noteStore.getClientName(selectedNote?.clientId) }}</p>
              <div class="flex items-center gap-3">
                <span class="text-[var(--color-text-body)] font-bold">{{ selectedNote?.contractId || '일반 상담' }}</span>
                <span class="h-1 w-1 rounded-full bg-[var(--color-text-placeholder)]"></span>
                <span class="text-[var(--color-text-sub)] text-sm">{{ selectedNote?.activityDate }}</span>
              </div>
            </div>
          </div>
          <div class="bg-[var(--color-bg-section)] p-8 rounded-2xl shadow-inner min-h-[300px]">
            <pre class="font-sans whitespace-pre-wrap text-[var(--color-text-body)] leading-loose text-base">{{ selectedNote?.content }}</pre>
          </div>
        </div>
        <div class="p-6 bg-[var(--color-bg-section)] border-t border-[var(--color-border-divider)] text-right">
          <button @click="closeDetail" class="h-12 px-8 bg-[var(--color-text-sub)] text-white rounded-xl font-bold hover:bg-[var(--color-text-body)] transition-all">창 닫기</button>
        </div>
      </div>
    </div>
  </section>
