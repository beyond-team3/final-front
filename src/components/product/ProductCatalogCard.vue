...
<template>
  <div
    class="overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-[0_1px_3px_rgba(61,53,41,0.06)] cursor-pointer transition-shadow hover:shadow-[0_4px_6px_rgba(61,53,41,0.08)] hover:border-[var(--color-olive)]"
    tabindex="0"
    role="button"
    @click="emit('select', item.id)"
    @keydown="onKeyActivate($event, item.id)"
  >
    <div class="relative h-48 bg-[var(--color-bg)]">
      <img
      ...
      <div v-if="showCompare || showFavorite" class="absolute right-2 top-2 flex gap-1">
        <button
          v-if="showCompare"
          type="button"
          class="rounded-full bg-[var(--color-surface)]/90 px-2 py-1 text-xs font-semibold"
          :class="compareActive ? 'text-[var(--color-olive-dark)] ring-1 ring-[var(--color-olive)]' : 'text-[var(--color-muted)]'"
        ...
        <button
          ...
          type="button"
          class="rounded-full bg-[var(--color-surface)]/90 px-2 py-1 text-xs font-semibold"
          :class="favoriteActive ? 'text-[var(--color-orange-dark)]' : 'text-[var(--color-muted)]'"
    ...
    <div class="block w-full p-4 text-left">
      <div class="flex items-center justify-between">
        ...
        <p v-if="showPrice && item.price" class="text-xs font-bold text-[var(--color-text)]">
          ₩{{ item.price.toLocaleString() }} <span class="font-normal text-[var(--color-muted)]">/ {{ item.amount || '' }}{{ item.unit }}</span>
        </p>
      </div>
      <h3 class="mt-1 text-base font-bold text-[var(--color-text)]">{{ item.name }}</h3>
      <p class="mt-2 line-clamp-2 text-sm text-[var(--color-muted)]">{{ item.desc }}</p>
    </div>
  </div>
</template>
