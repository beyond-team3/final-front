...
function normalizeDocumentSummary(doc = {}) {
  ...
  return {
    ...
    amount,
    remark: doc.memo || doc.requirements || doc.specialTerms || doc.remarks || '',
  }
}
...
function buildSteps(currentStageNumber) {
  return STEP_NAMES.map((name, index) => {
    const order = index + 1
    if (order < currentStageNumber) {
      return { name, state: 'completed', statusText: '완료' }
    }

    if (order === currentStageNumber) {
      return { name, state: 'in-progress', statusText: '진행중' }
    }
...
export const useHistoryStore = defineStore('history', () => {
  ...
  function createPipeline(client, document) {
    ...
    const next = {
      ...
      startDate: now,
      updatedAt: now,
      amount: Number(docSummary.amount || 0),
    ...
    createPipelineApi({
      ...
      error.value = getErrorMessage(e, '파이프라인 생성에 실패했습니다.')
    })
  ...
  function addDocumentToPipeline(historyId, document, newStage) {
    ...
    if (!pipeline) {
      if (normalizeType(document?.type) === 'quotation-request') {
        return createPipeline(document?.client, document)
      }
      return null
    }
    ...
    if (!pipeline.documents.find((item) => String(item.id) === String(docSummary.id))) {
      pipeline.documents.push(docSummary)
    }
    ...
    updatePipelineApi(pipeline.id, {
      ...
      error.value = getErrorMessage(e, '파이프라인 업데이트에 실패했습니다.')
    })
