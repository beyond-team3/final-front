...
위 입력이 부족하면, 합리적으로 가정하되 그 가정을 먼저 명시한다. 단, `LINEAR_ID`는 가정하지 않으며 반드시 제공하거나 `<MISSING_LINEAR_ID>` 고정 플레이스홀더를 사용해야 한다. `LINEAR_ID`가 누락된 상태에서 플레이스홀더도 없으면 커밋 메시지 생성을 중단하고 오류를 반환한다.
