...
const badgeClass = (category) => {
  if (category === '견적요청') return 'badge-quote'
  if (category === '계약') return 'badge-contract'
  if (category === '명세') return 'badge-spec'
...
<template>
  <section class="screen-content">
    <PageHeader title="알림" subtitle="거래처용 문서/계약 알림을 확인합니다." />
    ...
    <div class="noti-list-wrap">
      ...
      <footer class="noti-footer">
        <div class="pager">
          <button type="button" class="page-btn" :disabled="page === 1" @click="page -= 1">이전</button>
          <button
          ...
          <button type="button" class="page-btn" :disabled="page === totalPages" @click="page += 1">다음</button>
        </div>
...
.badge-spec { background: #f3e8ff; color: #6b21a8; }
.badge-invoice { background: #fff7ed; color: #9a3412; }
